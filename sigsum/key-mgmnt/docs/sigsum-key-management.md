...
started. This _signing-oracle TKey_ is plugged into a networked node
at all times (i.e., a primary log server, a secondary log server, or a
witness), and is only usable as a signing oracle if a passphrase (USS)
is made available.
...
Random Generator` and then base64 encode.
...
_signing-oracle_ TKey. A backup TKey reduces recovery time if the
signing-oracle needs to be replaced.
